...
const
  ...
  { DepartmentSupervisor } = require("../../model/db");
...
async function promise_user_list_data_for_rendering(args) {
  let
    company = args[0],
    users_info = args[1];
  // console.log(users_info);

  var usersInfoForRendering = [];
  // get all content from Department Supervisor table
  let department_supervisors = await DepartmentSupervisor.findAll();
  console.log(department_supervisors.length);

  // loop in all users data
  for (let i = 0; i < users_info.length; i++) {
    // for (let i = 25; i < 26; i++) {
    let ui = users_info[i];
    // get department supervisor user for every user
    let department_supervisor = await User.findById(ui.user_row.department.bossId);
    console.log(ui.user_row.department.id);

    // get second Supervisors from table Department Supervisor table -> user.department_id = DepartmentSupervisor.department_id
    var secondSupervisors = [];
    for (let i = 0; i < department_supervisors.length; i++) {
      let secondSupervisor = department_supervisors[i];
      // check if user department id = Department Supervisor department id
      if (secondSupervisor.department_id == ui.user_row.department.id) {
        // get user data of this second supervisor
        let Seconds = await User.findById(secondSupervisor.user_id);
        console.log(Seconds.full_name());
        // push the full name of this item to secondSupervisors array
        //secondSupervisors.push(Seconds.full_name());//
		secondSupervisors.push(Seconds.email);//
      }
    }
	  var strSecondSupervisors = secondSupervisors.join().replace(",","\n");
    // console.log(secondSupervisors);

    // handle secondSupervisors
    usersInfoForRendering.push(
      {
        user_id: ui.user_row.id,
        user_email: ui.user_row.email,
        user_name: ui.user_row.name,
        ...
        number_of_days_taken_from_allowance: ui.user_row.calculate_number_of_days_taken_from_allowance(),
        is_active: ui.user_row.is_active(),
        department_supervisor: ui.user_row.department.bossId,
        //department_supervisor_name: department_supervisor.full_name(),//
		department_supervisor_name: department_supervisor.email,//
        //department_second_supervisor_name: secondSupervisors//
	      department_second_supervisor_name: strSecondSupervisors
      }
    )
  }
...
function users_list_as_csv(args) {
  ...
  // Compose result CSV header
  //let content = [['email', 'lastname', 'name', 'department', 'supervisors', 'days used', 'department_supervisor_name', 'department_second_supervisor_name']];
let content = [['email', 'lastname', 'name', 'department', 'days used', 'department_supervisor_name', 'department_second_supervisor_name']];
  // ... and body
  users_info.forEach(ui => {
    content.push([
      ...
      //ui.department_name,
      ui.number_of_days_taken_from_allowance,
      ui.department_supervisor_name,
      ui.department_second_supervisor_name
    ]);
  });
